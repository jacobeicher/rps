...
class Board:
        ...
        if initial_value is not None:
            for row in range(self.height):
                for cell in range(self.width):
                    self.board[row][cell] = Cell(row, cell, initial_value[row][cell])
            return
    ...
    def get_copy(self):
        ...
        return Board(self.height, self.width, self.combat_mode, self.include_blanks, board_values)
...
def update_board(board, combat_mode = "random"):
    if combat_mode == "fixed":
        ...
        for row in range(board.get_size()[0]):
            for cell in range(board.get_size()[1]):
                board.update_cell(row, cell, board_reference)
        ...
        for cell in random.choices(cell_list, k=len(cell_list)):
                board.update_cell(cell.get_xy()[0], cell.get_xy()[1], board_reference)
